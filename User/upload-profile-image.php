...
include 'db_connect.php';  // Ensure the correct path to the database connection
session_start();
...
if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] == 0) {
    ...
    $imagePath = 'uploads/profile_images/' . $imageName;  // Relative path to the public uploads folder
    ...
    if (in_array($profileImage['type'], $allowedTypes)) {
        ...
        if (move_uploaded_file($profileImage['tmp_name'], $targetDir . $imageName)) {
            ...
            // Update the profile_picture in the database (store the relative path)
            $query = "UPDATE users SET profile_picture = ? WHERE id = ?";
            $stmt = $conn->prepare($query);  // Change $db to $conn
