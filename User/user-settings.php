...
<title>User Settings - Admin Dashboard</title>
...
<style>
  ...
  .modal-content {
    @apply bg-white rounded-lg shadow-lg max-w-md w-full;
  }
  ...
  .btn-primary {
    @apply bg-primary text-white hover:bg-blue-600 focus:ring-2 focus:ring-primary;
  }
  ...
  .btn-destructive {
    @apply bg-danger text-white hover:bg-red-600 focus:ring-2 focus:ring-red-500;
  }
...
<div class="flex min-h-screen">
   ...
   <div class="border-b border-gray-700 py-4 px-4">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 bg-red-50 rounded-md border border-red-100">
        ...
        <button id="delete-account-btn" class="btn btn-destructive transition-all duration-300">Request Deletion</button>
    </div>
</div>
                    </div>
                </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('password-form')?.addEventListener('submit', function (e) {
        ...
        if (newPassword !== confirmPassword) {
            // Show error modal for password mismatch
...
window.closeSuccessModal = function () {
    ...
    function showErrorModal(message) {
        ...
        errorModal.innerHTML = `
            <div class="modal-content p-6 mx-4 bg-white rounded-lg shadow-lg">
                ...
                <div class="flex justify-end gap-3">
                    <button class="btn btn-destructive" onclick="closeErrorModal()">Close</button>
                </div>
            </div>
        `;
    ...
    .then(data => {
        if (data.status === 'error') {
            ...
            // Update the request button UI
            const deleteAccountBtn = document.getElementById('delete-account-btn');
            if (deleteAccountBtn) {
                deleteAccountBtn.textContent = 'Request Sent';
                deleteAccountBtn.disabled = true;
                deleteAccountBtn.classList.remove('btn-destructive');
                deleteAccountBtn.classList.add('bg-gray-300', 'text-gray-700', 'cursor-not-allowed');
            }

            // Optional: You can also use a modal instead of alert
            showSuccessModal('Account deletion request submitted successfully.');
        }
    })
    .catch(error => {
