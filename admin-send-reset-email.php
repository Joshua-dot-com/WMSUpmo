...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        header("Location: admin_Login.php?error=" . urlencode("Please enter your email address."));
        exit;
    }
    ...
    if ($result->num_rows !== 1) {
        header("Location: admin_Login.php?error=" . urlencode("This email is not registered in the system."));
        exit;
    }
    ...
    if ($existing->num_rows > 0) {
        ...
        if ($expiresAt > $now) {
            ...
            header("Location: admin_Login.php?already_requested=1&minutes_remaining=$minutesLeft");
            exit;
        }
    }
    ...
    $resetLink = "http://localhost/PMO/admin-reset-password.php?token=$token";
    ...
    try {
        ...
        header("Location: admin_Login.php?error=" . urlencode("Failed to send reset email. Please try again."));
        exit;
    }
}
?>
